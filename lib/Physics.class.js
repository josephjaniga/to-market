"use strict";



class Physics{

    constructor(options){
        this.name = "Physics";

        this.gravityEnabled = true;
        this.gravity = {x:0, y:75};
    }

    Raycast(options){

        var hit = false,
            point = options.point,
            direction = options.direction,
            game = options.game,
            distance = 999999,
            ray = {},
            lines = [];

        // build this ray as a line
        ray = {
            start: {
                x:point.x,
                y:point.y
            },
            end: {
                x:point.x + direction.x * distance,
                y:point.y + direction.y * distance
            }
        };

        // assuming lines are in the format
        // {start:_, end:_, Collider:_}
        // where A & B are points
        // Collider is a reference to the collider that owns the segment

        var entities = [];

        // get all entities with colliders
        game.entities.forEach((entity)=>{
            var c = entity.GetComponent("Collider"),
                t = entity.GetComponent("Transform");
            if( c != null ){
                entities.push(t);
            }
        });

        // get all line segments that make up the box colliders
        entities.forEach((transform)=>{

            var p = transform.position,
                s = transform.size;

            /** the four points
             * A - - - B
             * |       |
             * |       |
             * |       |
             * C - - - D
             */
            var A={x:p.x, y:p.y},       B={x:p.x + s.x, y:p.y},
                C={x:p.x, y:p.y+s.y},   D={x:p.x+s.x, y:p.y+s.y};

            /**
             * the four exterior lines
             * A-B, B-D, C-D, A-C
             */

            var AB = {start:A, end:B},
                BD = {start:B, end:D},
                CD = {start:C, end:D},
                AC = {start:A, end:C};

            lines.push(AB, BD, CD, AC);
        });

        // check intersection against all the lines
        hit = lines.some((line)=>{
            return isIntersect(ray.start, ray.end, line.start, line.end);
        });

        game.debug.add({
            type: "Raycast",
            ray: ray,
            result: hit
        });

        return hit;
    }

    isIntersect(p1, p2, p3, p4) {
        return (CCW(p1, p3, p4) != CCW(p2, p3, p4)) && (CCW(p1, p2, p3) != CCW(p1, p2, p4));
    }

}

module.exports = Physics;


//assuming points are in the format {x:_, y;_}
function CCW(p1, p2, p3){
    var a = p1.x, b = p1.y,
        c = p2.x, d = p2.y,
        e = p3.x, f = p3.y;
    return (f - b) * (c - a) > (d - b) * (e - a);
}